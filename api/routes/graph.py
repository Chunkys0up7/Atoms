...
def get_full_graph() -> Dict[str, Any]:
    ...
    paths = [
        Path("docs/generated/api/graph/full.json"),
        Path("docs/api/graph/full.json")
    ]
    ...
    raise HTTPException(
        status_code=404,
        detail="Graph not found. Run 'python builder.py build' to generate it."
    )
...
def get_graph_by_type(atom_type: str) -> Dict[str, Any]:
    ...
    paths = [
        Path(f"docs/generated/api/graph/type/{atom_type}.json"),
        Path(f"docs/api/graph/type/{atom_type}.json")
    ]
    ...
    raise HTTPException(
        status_code=404,
        detail=f"Graph for type '{atom_type}' not found. Run 'python builder.py build' to generate it."
    )
...
def get_graph_by_module(module_id: str) -> Dict[str, Any]:
    ...
    paths = [
        Path(f"docs/generated/api/graph/module/{module_id}.json"),
        Path(f"docs/api/graph/module/{module_id}.json")
    ]
