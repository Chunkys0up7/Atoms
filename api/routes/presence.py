...
def get_user_presence(user_id: str) -> UserPresence:
    ...
    if not user_info:
        raise HTTPException(status_code=404, detail=f"User {user_id} not found or offline")
...
def get_room_members(room_id: str) -> RoomInfo:
    ...
    return RoomInfo(room_id=room_id, member_count=len(members), members=members)
...
def send_heartbeat(user_id: str) -> Dict[str, Any]:
    ...
    if user_info:
        ...
        return {"status": "ok", "user_id": user_id, "last_seen": datetime.now().isoformat()}
    else:
        raise HTTPException(status_code=404, detail=f"User {user_id} not found in presence system")
...
def get_all_rooms() -> List[RoomInfo]:
    ...
    for room_id, member_count in stats["rooms"].items():
        ...
        rooms.append(RoomInfo(room_id=room_id, member_count=member_count, members=members))
