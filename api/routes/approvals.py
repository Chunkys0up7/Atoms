...
def request_atom_usage_approval(request: AtomUsageApprovalRequest) -> Dict[str, Any]:
    ...
    if not owner:
        raise HTTPException(status_code=404, detail=f"Atom '{request.atom_id}' not found or has no owner")
...
def request_module_approval(request: ModuleApprovalRequest) -> Dict[str, Any]:
    ...
    for atom_id in atom_ids:
        ...
        for request_file in approvals_dir.glob("*.json"):
            ...
            if (
                approval_request.get("type") == "atom_usage"
                and approval_request.get("atom_id") == atom_id
                and approval_request.get("module_id") == request.module_id
                and approval_request.get("status") == "approved"
            ):
                approved = True
                break
    ...
    for atom_id in atom_ids:
        ...
        for request_file in approvals_dir.glob("*.json"):
            ...
            if (
                approval_request.get("type") == "atom_usage"
                and approval_request.get("atom_id") == atom_id
                and approval_request.get("module_id") == module_id
            ):
        ...
        atom_approval_status.append(
            {"atom_id": atom_id, "owner": owner, "approved": approved, "approval_id": approval_id}
        )
