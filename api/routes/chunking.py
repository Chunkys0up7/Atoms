...
async def chunk_document(request: ChunkRequest):
    ...
    if request.chunk_strategy == "semantic":
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Unknown chunking strategy: {request.chunk_strategy}"
        )
...
def chunking_health():
    ...
    return {
        ...
        "strategies_available": ["semantic", "paragraph", "fixed_size"] if HAS_TRANSFORMERS else ["paragraph", "fixed_size"]
    }
