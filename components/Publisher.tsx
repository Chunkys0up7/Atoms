...
const Publisher: React.FC<PublisherProps> = ({ atoms, modules }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 overflow-hidden">
      <div className="p-10 border-b border-slate-800 bg-slate-900/40 flex items-center justify-between shrink-0">
        <div>
          <h2 className="text-4xl font-black text-white tracking-tight mb-2 uppercase">AI Document Compiler</h2>
          <p className="text-slate-500 text-sm font-medium">Synthesize atomic units into cohesive professional artifacts via LLM.</p>
        </div>
        ...
        <div className="flex gap-4">
           ...
           <button 
             ...
             className="bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 text-white px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-blue-900/30 transition-all flex items-center gap-3"
           >
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Module Picker */}
        <div className="w-80 border-r border-slate-800 bg-slate-900/20 p-8 flex flex-col gap-6 overflow-y-auto custom-scrollbar">
           ...
           <div className="space-y-3">
             {modules.map(mod => (
               <button
                 ...
                 className={`w-full text-left p-4 rounded-2xl border transition-all ${selectedModuleId === mod.id ? 'bg-blue-600/10 border-blue-500 text-blue-400' : 'bg-slate-900/40 border-slate-800 text-slate-500 hover:border-slate-700'}`}
               >
        ...
        {/* Content Preview */}
        <div className="flex-1 bg-slate-950 p-12 overflow-y-auto custom-scrollbar relative">
          {compiledText ? (
            <div className="max-w-4xl mx-auto">
              <div className="flex justify-between items-center mb-10 pb-6 border-b border-slate-800">
                 <h3 className="text-2xl font-black text-white tracking-tight">Compiled Output Preview</h3>
                 <button 
                  onClick={handleDownload}
                  className="bg-slate-800 hover:bg-slate-700 text-slate-300 px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border border-slate-700 flex items-center gap-2"
                 >
            ...
            <div className="h-full flex flex-col items-center justify-center text-center opacity-40">
               ...
               <h3 className="text-xl font-black text-white">Compiler Standby</h3>
               <p className="text-slate-500 max-w-xs mt-2">Select a module and template, then click Compile to generate a polished enterprise document.</p>
            </div>
          )}
        </div>
      </div>
