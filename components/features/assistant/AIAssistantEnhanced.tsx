...
const AIAssistantEnhanced: React.FC<AIAssistantProps> = ({ atoms }) => {
  ...
  const checkRagHealth = async () => {
    try {
      const response = await fetch(API_ENDPOINTS.rag.health);
      if (response.ok) {
  ...
  const fetchRagMetrics = async () => {
    try {
      const response = await fetch(API_ENDPOINTS.rag.metrics);
      if (response.ok) {
        ...
        <div className="grid grid-cols-3 gap-2">
          {(['entity', 'path', 'impact'] as RAGMode[]).map(mode => (
            <button
              ...
              className={`p-3 rounded-lg border transition-all ${ragMode === mode
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] rounded-[2rem] p-5 ${msg.role === 'user'
